...
if __name__=='__main__':
    ...
    try:
        ...
        if os.path.exists(main_audio):  #Case base audio has audio
            ...
            ffmpeg_cmd = f"""ffmpeg -y {input_file} -filter_complex "{filer_complex_str}{amix}amix={len(list_audio_path)+1},volume=2" -c:v copy {map_str} {save_path}"""
            ...
            for i  in range(len(list_audio_path)):
                ...
                amix = amix + f"[aud{i+1}]"
                # map_str = map_str + f' -map {i+2}:a'
            ffmpeg_cmd = f"""ffmpeg -y {input_file} -filter_complex "{filer_complex_str}{amix}amix={len(list_audio_path)}" -c:a aac -c:v copy  {save_path}"""
            print("FFMPEG COMMAND: ",ffmpeg_cmd)
            os.system(ffmpeg_cmd)
